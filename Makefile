##########################################################################################################################
# File automatically-generated by tool: [projectgenerator] version: [4.4.0-B60] date: [Fri Mar 28 16:18:29 EDT 2025] 
##########################################################################################################################
...
-IMiddlewares/ST/lib_gnss/LibNMEA/Inc \
...
#######################################
# link script
LDSCRIPT = STM32H753IITx_FLASH.ld

# use a custom linker script so we can move program code away from DTCM
LDSCRIPT_CUSTOM = STM32H753IITx_FLASH_custom.ld

# libraries
LIBS = -lc -lm -lnosys 
LIBDIR = 
LDFLAGS = $(MCU) -specs=nano.specs -T$(LDSCRIPT_CUSTOM) $(LIBDIR) $(LIBS) -Wl,-Map=$(BUILD_DIR)/$(TARGET).map,--cref -Wl,--gc-sections -u _printf_float
...
OBJECTS_NO_OPT = $(addprefix $(BUILD_DIR)/,$(notdir $(C_SOURCES_OPT_HIGH:.c=.o)))
OBJECTS += $(OBJECTS_NO_OPT)
vpath %.c $(sort $(dir $(C_SOURCES_OPT_HIGH)))
