...
int32_t BSP_I2C3_Send_IT(uint16_t DevAddr, uint8_t *pData, uint16_t Length)
{
  ...
  if(HAL_I2C_Master_Transmit_IT(&hi2c3, (uint8_t)DevAddr,
...
int32_t BSP_I2C3_Recv_IT(uint16_t DevAddr, uint8_t *pData, uint16_t Length)
{
  ...
  if(HAL_I2C_Master_Receive_IT(&hi2c3, (uint8_t)DevAddr,
                                        (uint8_t *)pData, Length) == HAL_OK)
  {
    ret = BSP_ERROR_NONE;
  }
...
int32_t BSP_I2C3_RegisterRxCallback(pI2C_CallbackTypeDef pCallback)
{
  ...
  return HAL_I2C_RegisterCallback(&hi2c3, HAL_I2C_MASTER_RX_COMPLETE_CB_ID, pCallback);
}
...
int32_t BSP_I2C3_RegisterErrorCallback(pI2C_CallbackTypeDef pCallback)
{
  ...
  return HAL_I2C_RegisterCallback(&hi2c3, HAL_I2C_ERROR_CB_ID, pCallback);
}
...
int32_t BSP_I2C3_RegisterAbortCallback(pI2C_CallbackTypeDef pCallback)
{
  ...
  return HAL_I2C_RegisterCallback(&hi2c3, HAL_I2C_ABORT_CB_ID, pCallback);
}
#endif
